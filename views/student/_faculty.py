...
class FacultyFrame(ctk.CTkFrame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.FacultyDataWrapper = ctk.CTkScrollableFrame(master=self.FacultyWrapper, fg_color="transparent")
    ...
    def DisplayFaculty(self, asc: str = "Ascending") -> None:
            ...
            for idx in range(len(fetched_data)):
                ...
                # Main frame
                self._cache_frame[idx] = ctk.CTkFrame(master=self.FacultyDataWrapper, fg_color="transparent", border_color=res.constants.THEME_GRAY, border_width=2, corner_radius=3)
                ...
                #Inner Teacher Text
                ctk.CTkLabel(master=self._cache_teacher_frame[idx], text=f"{fetched_data[idx]['username']}", text_color=res.constants.THEME_TEXT, font=ctk.CTkFont(family=res.fonts.POPPINS, size=20, weight='bold')).grid(row=0, column=0, sticky="nsew")
                ...
                ctk.CTkLabel(master=self._cache_teacher_frame[idx], text=f"Time period: {formatted_time}", text_color=res.constants.THEME_BNW, font=ctk.CTkFont(family=res.fonts.POPPINS, size=11)).grid(row=1, column=0, sticky="nsew")
                # Status if Open or Reserved by tbl_faculty
                ctk.CTkLabel(master=self._cache_info_frame[idx], text=f"{fetched_data[idx]['status']}", text_color=res.constants.THEME_BNW, font=ctk.CTkFont(family=res.fonts.POPPINS, size=12)).grid(row=0, column=0, sticky="nsew")
                # Button GG go next
                ctk.CTkButton(master=self._cache_frame[idx], command=lambda key=idx: self.DisplayFacultyInfo(schedule_id=fetched_data[key]['schedule_id'], schedule=fetched_data[key]['scheduled_on']), image=self.master.GoNextImage, text=None, fg_color="transparent", hover=None).grid(row=0, column=3, padx=10, pady=10, sticky="e")
