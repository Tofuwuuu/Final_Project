...
class DashboardFrame(ctk.CTkFrame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.SearchUtilityWrapper.grid(row=1, column=0, columnspan=1, padx=5, pady=5, sticky="nsew")
    ...
    def UpdateUpcoming(self, asc: str = "Ascending") -> None:
        ...
        for idx in range(len(account_history)):
            ...
            #Inner Teacher Text
            ctk.CTkLabel(master=self._cache_teacher_frame[idx], text=f"{account_history[idx]['prefix']}. {account_history[idx]['teacher']}", text_color=("#2B9348", "#Fdf0d5"), font=ctk.CTkFont(family="Poppins", size=20, weight='bold')).grid(row=0, column=0, sticky="nsew")
            ...
            # Inner TimeSpan in TeacherWrapper
            ctk.CTkLabel(master=self._cache_info_frame[idx], text=f"    {account_history[idx]['status']}", text_color=("black", "white"), font=ctk.CTkFont(family="Poppins", size=12)).grid(row=1, column=0, padx=30, sticky="w")
            # Button gg go next
            ctk.CTkButton(master=self._cache_frame[idx], command=lambda:self.master.SelectedPanel("consultation"),image=self.GoNextImage, text=None, fg_color="transparent", hover=None).grid(row=0, column=3, padx=5, pady=10, sticky="e")