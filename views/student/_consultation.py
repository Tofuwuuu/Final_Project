...
class ConsultationFrame(ctk.CTkFrame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.SearchButton = ctk.CTkButton(master=self.SearchUtilityWrapper, command=self.UpdateDisplay, width=90, height=33, text="Search", image=self.SearchImage, compound="right", text_color=self.THEME_YELLOW, font=ctk.CTkFont(family="Poppins", size=13), fg_color="#2B9348", hover_color="#55A630")
        ...
        self.Sort = ctk.CTkComboBox(master=self.SearchUtilityWrapper, command=lambda value: self.UpdateDispUpdateDisplaylay(asc=value), values=["Ascending", "Descending"], variable=self.SortVariable, height=30, text_color=("#242424", 'white'), font=ctk.CTkFont(family="Poppins", size=11), dropdown_fg_color=self.THEME_YELLOW, fg_color=self.DEFAULT, border_color=self.THEME_GREEN, border_width=2, corner_radius=5, button_hover_color="#55A630", button_color=self.THEME_GREEN)
    ...
    def UpdateDisplay(self, asc: str = "Ascending") -> None:
        ...
        if asc == "Ascending":
            account_history = sorted(account_history, key=lambda x: str(x["scheduled_on"]))
        elif asc == "Descending":
            account_history = sorted(account_history, key=lambda x: str(x["scheduled_on"]), reverse=True)
        ...
        for idx in range(len(account_history)):
            ...
            #Inner Teacher Text
            ctk.CTkLabel(master=self._cache_teacher_frame[idx], text=f"{account_history[idx]['teacher']}", text_color=("#2B9348", "#Fdf0d5"), font=ctk.CTkFont(family="Poppins", size=20, weight='bold')).grid(row=0, column=0, sticky="nsew")
            ...
            ctk.CTkLabel(master=self._cache_teacher_frame[idx], text=f"Time: {formatted_time} | Status: {account_history[idx]['status']}", text_color=("black", "white"), font=ctk.CTkFont(family="Poppins", size=12)).grid(row=1, column=0, sticky="w")
            # Inner TimeSpan in TeacherWrapper
            ...
            # Button gg go next
            ctk.CTkButton(master=self._cache_frame[idx], command=lambda:self.master.SelectedPanel("history"),image=self.GoNextImage, text=None, fg_color="transparent", hover=None).grid(row=0, column=4, padx=5, pady=10, sticky="e")