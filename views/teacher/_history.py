...
class HistoryFrame(ctk.CTkFrame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.SearchButton = ctk.CTkButton(master=self.SearchUtilityWrapper, command=self.UpdateHistory, width=90, height=33, text="Search", image=self.SearchImage, compound="right", text_color=self.THEME_YELLOW, font=ctk.CTkFont(family="Poppins", size=13), fg_color="#2B9348", hover_color="#55A630")
        ...
        self.Sort = ctk.CTkComboBox(master=self.SearchUtilityWrapper, command=lambda value: self.UpdateDispUpdateHistorylay(asc=value), values=["Ascending", "Descending"], variable=self.SortVariable, height=30, text_color=("#242424", 'white'), font=ctk.CTkFont(family="Poppins", size=11), dropdown_fg_color=self.THEME_YELLOW, fg_color=self.DEFAULT, border_color=self.THEME_GREEN, border_width=2, corner_radius=5, button_hover_color="#55A630", button_color=self.THEME_GREEN)
    ...
    def UpdateHistory(self, asc: str = "Ascending") -> None:
        ...
        for idx in range(len(account_history)):
            ...
            ctk.CTkLabel(master=self._cache_teacher_frame[idx], text=f"Time: {formatted_time} | Status: {account_history[idx]['request_status']}", text_color=("black", "white"), font=ctk.CTkFont(family="Poppins", size=12)).grid(row=1, column=0, sticky="w")
            # Inner TimeSpan in TeacherWrapper
