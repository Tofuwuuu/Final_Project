...
class HistoryFrame(ctk.CTkFrame):
    ...
    def UpdateHistory(self, asc: str = "Ascending") -> None:
        ...
        for idx in range(len(account_history)):
            ...
            ctk.CTkLabel(master=_cache_teacher_frame[idx], text=f"Time: {formatted_time} | Status: {account_history[idx]['request_status']}", text_color=("black", "white"), font=ctk.CTkFont(family="Poppins", size=12)).grid(row=1, column=0, sticky="w")
            # Inner TimeSpan in TeacherWrapper
            ...
            # Button gg go next
            ctk.CTkButton(master=_cache_frame[idx], command=lambda:self.master.SelectedPanel("history"),image=self.GoNextImage, text=None, fg_color="transparent", hover=None).grid(row=0, column=4, padx=5, pady=10, sticky="e")